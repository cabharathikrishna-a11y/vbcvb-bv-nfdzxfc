...
sealed interface LogoutFlowState {
    object Idle : LogoutFlowState
    object GeneratingBackup : LogoutFlowState
    data class BackupSuccessAndWiping(val fileName: String) : LogoutFlowState
    data class Completed(val fileName: String) : LogoutFlowState
    data class Error(val message: String) : LogoutFlowState
}
...
class AppViewModel(
    ...
    fun setShowLogoutConfirm(show: Boolean) {
        _showLogoutConfirm.value = show
        if (!show) {
            _logoutFlowState.value = LogoutFlowState.Idle
        }
    }

    private val _logoutFlowState = MutableStateFlow<LogoutFlowState>(LogoutFlowState.Idle)
    val logoutFlowState: StateFlow<LogoutFlowState> = _logoutFlowState.asStateFlow()

    fun resetLogoutFlowState() {
        _logoutFlowState.value = LogoutFlowState.Idle
    }
    ...
    private val _focusKeepScreenOn = MutableStateFlow(true)
    val focusKeepScreenOn: StateFlow<Boolean> = _focusKeepScreenOn.asStateFlow()

    private val _autoUltraLowBrightnessInFullScreen = MutableStateFlow(true)
    val autoUltraLowBrightnessInFullScreen: StateFlow<Boolean> = _autoUltraLowBrightnessInFullScreen.asStateFlow()

    private val _focusBrightnessMode = MutableStateFlow("default")
    val focusBrightnessMode: StateFlow<String> = _focusBrightnessMode.asStateFlow()

    private val _focusBrightnessCustom = MutableStateFlow(0.25f)
    val focusBrightnessCustom: StateFlow<Float> = _focusBrightnessCustom.asStateFlow()

    private val _autoFullscreenOnHomeIdle = MutableStateFlow(true)
    val autoFullscreenOnHomeIdle: StateFlow<Boolean> = _autoFullscreenOnHomeIdle.asStateFlow()

    private val _autoFullscreenIdleSeconds = MutableStateFlow(10)
    val autoFullscreenIdleSeconds: StateFlow<Int> = _autoFullscreenIdleSeconds.asStateFlow()

    private val _autoFullscreenZoomAnim = MutableStateFlow(true)
    val autoFullscreenZoomAnim: StateFlow<Boolean> = _autoFullscreenZoomAnim.asStateFlow()
    ...
    fun loadWaterGlassesToday() {
        ...
        if (savedDate != todayStr) {
            ...
            _waterGlassesToday.value = prefs.getSafeInt("water_glasses_count", 0)
        }
    }
    ...
    fun updateFocusKeepScreenOn(enabled: Boolean) {
        _focusKeepScreenOn.value = enabled
        prefs.edit().putBoolean(com.example.util.FocusDisplayManager.KEY_FOCUS_KEEP_SCREEN_ON, enabled).apply()
    }

    fun updateAutoUltraLowBrightnessInFullScreen(enabled: Boolean) {
        _autoUltraLowBrightnessInFullScreen.value = enabled
        prefs.edit().putBoolean(com.example.util.FocusDisplayManager.KEY_AUTO_ULTRA_LOW_BRIGHTNESS, enabled).apply()
    }

    fun updateFocusBrightnessMode(mode: String) {
        _focusBrightnessMode.value = mode
    }

    fun updateFocusBrightnessCustom(level: Float) {
        _focusBrightnessCustom.value = level
    }

    fun updateAutoFullscreenOnHomeIdle(enabled: Boolean) {
        _autoFullscreenOnHomeIdle.value = enabled
        prefs.edit().putBoolean(com.example.util.FocusDisplayManager.KEY_AUTO_FULLSCREEN_ON_HOME_IDLE, enabled).apply()
    }

    fun updateAutoFullscreenIdleSeconds(seconds: Int) {
        val bounded = seconds.coerceIn(3, 120)
        _autoFullscreenIdleSeconds.value = bounded
        prefs.edit().putInt(com.example.util.FocusDisplayManager.KEY_AUTO_FULLSCREEN_IDLE_SECONDS, bounded).apply()
    }

    fun updateAutoFullscreenZoomAnim(enabled: Boolean) {
        _autoFullscreenZoomAnim.value = enabled
        prefs.edit().putBoolean(com.example.util.FocusDisplayManager.KEY_AUTO_FULLSCREEN_ZOOM_ANIM, enabled).apply()
    }
    ...
    fun performLogoutWithRestorationAndWipe(context: android.content.Context, activity: android.app.Activity? = null) {
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            _logoutFlowState.value = LogoutFlowState.GeneratingBackup
            
            // 1. Create and verify backup in Downloads folder
            val (backupSuccess, fileName) = com.example.util.DatabaseBackupHelper.createAndSaveRestorationBackupToDownloads(
                context, repository.db
            )

            if (!backupSuccess || fileName.isBlank()) {
                _logoutFlowState.value = LogoutFlowState.Error("Could not save restoration file to Downloads folder. Local data was preserved.")
                return@launch
            }

            // 2. State transition: notify user that backup was saved to Downloads and wiping begins
            _logoutFlowState.value = LogoutFlowState.BackupSuccessAndWiping(fileName)
            kotlinx.coroutines.delay(1200)

            // 3. Complete device wipe: delete all user databases, shared prefs, cache, files, listeners, tokens
            com.example.util.DatabaseBackupHelper.wipeAllLocalUserData(context, repository.db)

            // 4. Reset VM memory states
    ...
    fun exitAndCloseApp(activity: android.app.Activity? = null) {
        try {
    ...
    fun transcribeVoiceNoteToJournal(
        ...
        viewModelScope.launch {
            try {
                ...
                val title = "Voice Journal - ${java.text.SimpleDateFormat("MMM dd, yyyy HH:mm", java.util.Locale.US).format(java.util.Date())}"
                val content = "Voice audio note recorded. (AI transcription models removed)"
    ...
    fun transcribeAudioFileOnly(
        ...
        viewModelScope.launch {
            try {
                ...
                onSuccess("Voice audio note (${audioFile.name})")
            } catch (e: Exception) {
    ...
    fun loadContactFolders() {
        val foldersSet = prefs.getSafeStringSet("contact_folders_set", emptySet()) ?: emptySet()
        ...
        return buildString {
            ...
            if (leaderboard.isNotEmpty()) {
                ...
                leaderboard.take(10).forEach { rankModel ->
                    ...
                    append("  - #" + rankModel.rank + " " + rankModel.displayName + meSuffix + ": " + mins + " mins focused\n")
                }
            } else {
            ...
            return buildString {
                ...
                if (leaderboard.isNotEmpty()) {
                    leaderboard.take(15).forEach { rankModel ->
                        ...
                        append("#${rankModel.rank} **${rankModel.displayName}**$meTag: **${mins} mins** focused$subjectInfo\n")
                    }
                } else {
            ...
            when (intentResult) {
                is com.example.util.NlpIntentResult.TaskCreation -> {
                    createTask(
                        ...
                        estMin = intentResult.estMin,
                        category = intentResult.category,
                        dueDateString = intentResult.dueDateString
                    )
                    actionConfirmation = "✅ **Task Created Successfully!**\n- **Title:** ${intentResult.title}\n- **Due Date:** ${intentResult.dueDateString}\n- **Category:** ${intentResult.category}\n- **Est. Duration:** ${intentResult.estMin} mins"
                }
                is com.example.util.NlpIntentResult.FinanceLogging -> {
                    createLedgerEntry(
                        type = intentResult.type,
                        amount = intentResult.amount,
                        tag = intentResult.tag,
                        note = intentResult.note
                    )
                    val symbol = "₹"
                    actionConfirmation = "💰 **Finance Logged Successfully!**\n- **Type:** ${intentResult.type}\n- **Amount:** $symbol${intentResult.amount}\n- **Category/Tag:** ${intentResult.tag}\n- **Note:** ${intentResult.note}"
                }
                is com.example.util.NlpIntentResult.ScheduleChange -> {
                    val matchingTask = tasks.value.find { it.title.contains(intentResult.targetTitle, ignoreCase = true) }
                    if (matchingTask != null) {
                        when (intentResult.changeType) {
                            com.example.util.NlpIntentResult.ScheduleChangeType.COMPLETE_TASK -> {
                                if (!matchingTask.isCompleted) toggleTaskCompletion(matchingTask)
                                actionConfirmation = "✅ **Task Completed!** Marked '${matchingTask.title}' as done."
                            }
                            com.example.util.NlpIntentResult.ScheduleChangeType.DELETE_TASK -> {
                                deleteTask(matchingTask)
                                actionConfirmation = "🗑️ **Task Deleted!** Removed '${matchingTask.title}'."
                            }
                            com.example.util.NlpIntentResult.ScheduleChangeType.RESCHEDULE_DUE_DATE -> {
                                val newDate = intentResult.newDueDateString ?: getCurrentDateString()
                                updateTask(matchingTask.copy(dueDateString = newDate))
                                actionConfirmation = "📅 **Task Rescheduled!** Moved '${matchingTask.title}' to $newDate."
                            }
                            com.example.util.NlpIntentResult.ScheduleChangeType.CHANGE_PRIORITY -> {
                                val prio = intentResult.newPriority ?: "MEDIUM"
                                updateTask(matchingTask.copy(priority = prio))
                                actionConfirmation = "⚡ **Priority Updated!** Set '${matchingTask.title}' priority to $prio."
                            }
                            else -> {
                                actionConfirmation = "✅ **Task Updated!** Modified '${matchingTask.title}'."
                            }
                        }
                    } else {
                        actionConfirmation = "🔍 Could not find active task matching '${intentResult.targetTitle}'."
                    }
                }
                is com.example.util.NlpIntentResult.JournalEntry -> {
                    createJournalEntry(title = intentResult.title, text = intentResult.content)
                    actionConfirmation = "📖 **Journal Entry Saved!**\n- **Title:** ${intentResult.title}\n- **Content:** ${intentResult.content}"
                }
                else -> {
                    // Check for custom app queries
                    val lower = userText.lowercase(java.util.Locale.getDefault())
                    if (lower.contains("pending task") || lower.contains("my tasks") || lower.contains("what tasks")) {
                        val pending = tasks.value.filter { !it.isCompleted }
                        actionConfirmation = if (pending.isNotEmpty()) {
                            "📋 **Your Active Pending Tasks (${pending.size}):**\n" + pending.take(8).mapIndexed { idx, t -> "${idx + 1}. **${t.title}** (Due: ${t.dueDateString.ifEmpty { "Today" }}, Priority: ${t.priority})" }.joinToString("\n")
                        } else {
                            "🎉 All clear! You have zero pending tasks."
                        }
                    } else if (lower.contains("habit streak") || lower.contains("my habits")) {
                        val activeHabits = habits.value
                        actionConfirmation = if (activeHabits.isNotEmpty()) {
                            "🎯 **Your Active Habits (${activeHabits.size}):**\n" + activeHabits.take(6).mapIndexed { idx, h -> "${idx + 1}. **${h.name}** • Frequency: ${h.frequency}" }.joinToString("\n")
                        } else {
                            "No habits tracked yet. Type 'Create habit 30m reading' to add one!"
                        }
                    }
                }
            }
    ...
    fun triggerJournalSummarization(journalId: Int) {
        ...
        _chatbotMessages.value = _chatbotMessages.value + ChatMessage(
            text = "AI summarization for '${entry.title}' removed.",
            isUser = false,
            modelUsed = "None"
        )
        navigateTo(Screen.DEEPA_AI)
    ...
    fun runAdvancedAIFinancialAudit() {
        navigateTo(Screen.DEEPA_AI)
        _chatbotMessages.value = _chatbotMessages.value + ChatMessage(
            text = "AI Financial Audit models have been removed from this build.",
            isUser = false,
            modelUsed = "None"
        )
        _chatbotLoading.value = false
        ...
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.Default) {
            while (true) {
                try {
                    ...
                    val optimisticSecs = com.example.util.FocusTimerManager.optimisticTodayFocusSeconds.value
                    val baseTotalSecs = completedTodaySecs + pendingSecs + (myLocalActiveMs / 1000L).toInt()
                    val trueTotalSecs = if (optimisticSecs != null) maxOf(baseTotalSecs.toLong(), optimisticSecs) else baseTotalSecs.toLong()
                    val myLocalTrueTotalMs = trueTotalSecs * 1000L
                    val myLocalCompletedMs = (if (hasActiveSession) (trueTotalSecs - (myLocalActiveMs / 1000L)).coerceAtLeast(0L) else trueTotalSecs) * 1000L
    ...
    init {
                    ...
                    if (!isLocalActiveOrPaused) {
                        ...
                        if (status.lowercase() != "idle" && status.isNotEmpty()) {
                            ...
                            if (mode.lowercase() == "stopwatch") {
                                ...
                                val focusMinsSetting = prefs.getSafeInt("pomodoro_focus_duration_mins", 25)
        ...
        val previouslyLoggedIn = prefs.getSafeBoolean("is_logged_in", false)
        val savedUsername = prefs.getSafeString("current_username", null)
        ...
        if (googleAccount != null || (previouslyLoggedIn && savedUsername != null)) {
            ...
            val cachedName = prefs.getSafeString("user_name_${username}", "") ?: ""
            val cachedNickname = prefs.getSafeString("user_nickname_${username}", "") ?: ""
            val cachedEmoji = prefs.getSafeString("user_emoji_${username}", "") ?: ""
            val cachedEmail = prefs.getSafeString("user_email_${username}", "") ?: ""
            ...
            val email = googleAccount?.email ?: prefs.getSafeString("user_email_${username}", "") ?: ""
            ...
            viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
                ...
                // Daily check of logged in Gmail/Google accounts for freshest profile picture
                com.example.util.ProfilePictureManager.performDailyProfilePicSync(getApplication(), force = false)
            }
        } else {
            _isLoggedIn.value = false
            _currentUsername.value = null
        ...
        val savedMemories = prefs.getSafeString("ai_memories_list", "") ?: ""
        ...
        _focusTimerDurationMins.value = prefs.getSafeInt("timer_duration", 25)
        _breakDurationMins.value = prefs.getSafeInt("break_duration", 5)
        _soundOption.value = prefs.getSafeString("sound_option", "Raindrops") ?: "Raindrops"
        _vibrationEnabled.value = prefs.getSafeBoolean("vibration_enabled", true)
        _shareFocusDetailsEnabled.value = prefs.getSafeBoolean("share_focus_details_enabled", true)
        _shareFocusHistoryEnabled.value = prefs.getSafeBoolean("share_focus_history_enabled", true)
        _focusMotivationalQuoteEnabled.value = prefs.getSafeBoolean("focus_motivational_quote_enabled", false)
        _focusMotivationalQuoteIntervalMins.value = prefs.getSafeInt("focus_motivational_quote_interval_mins", 5)

        _waterReminderEnabled.value = prefs.getSafeBoolean("water_reminder_enabled", false)
        _waterReminderIntervalMins.value = prefs.getSafeFloat("water_reminder_interval_mins", 60.0f)
        _waterReminderStartTime.value = prefs.getSafeString("water_reminder_start_time", "08:00") ?: "08:00"
        _waterReminderEndTime.value = prefs.getSafeString("water_reminder_end_time", "22:00") ?: "22:00"

        _defaultStepGoal.value = prefs.getSafeInt("default_step_goal", 10000)
        _defaultSleepGoalMinutes.value = prefs.getSafeInt("default_sleep_goal_minutes", 480)
        _defaultWaterGoalMl.value = prefs.getSafeInt("default_water_goal_ml", 2000)

        _tabBarOrientation.value = prefs.getSafeString("tab_bar_orientation", "vertical") ?: "vertical"
        _taskVibrationEnabled.value = prefs.getSafeBoolean("task_vibration_enabled", true)
        _additionalReminderTimes.value = prefs.getSafeString("additional_reminder_times", "") ?: ""
        _antiBurnScreenEnabled.value = prefs.getSafeBoolean("anti_burn_screen_enabled", false)
        _hideStatusBarInFullScreen.value = prefs.getSafeBoolean("hide_status_bar_in_fullscreen", true)
        _showClockTimeInFullScreen.value = prefs.getSafeBoolean("show_clock_time_in_fullscreen", true)
        _showBatteryInFullScreen.value = prefs.getSafeBoolean("show_battery_in_fullscreen", true)
        _focusKeepScreenOn.value = prefs.getSafeBoolean(com.example.util.FocusDisplayManager.KEY_FOCUS_KEEP_SCREEN_ON, true)
        _autoUltraLowBrightnessInFullScreen.value = prefs.getSafeBoolean(com.example.util.FocusDisplayManager.KEY_AUTO_ULTRA_LOW_BRIGHTNESS, true)
        _autoFullscreenOnHomeIdle.value = prefs.getSafeBoolean(com.example.util.FocusDisplayManager.KEY_AUTO_FULLSCREEN_ON_HOME_IDLE, true)
        ...
        _instagramMessagesBlocked.value = prefs.getSafeBoolean("ig_messages_blocked", false)
        _instagramExploreBlocked.value = prefs.getSafeBoolean("ig_explore_blocked", true)
        ...
        _youtubeSearchBlocked.value = prefs.getSafeBoolean("yt_search_blocked", false)
        _youtubeCommentsBlocked.value = prefs.getSafeBoolean("yt_comments_blocked", false)
        _batterySaverModeEnabled.value = prefs.getSafeBoolean("battery_saver_mode", false)
        _showOverlayEnabled.value = prefs.getSafeBoolean("show_overlay_on_exit", true)
        _floatingTimerSize.value = prefs.getSafeString("floating_timer_size", "large") ?: "large"
        _keepNotificationEnabled.value = prefs.getSafeBoolean("keep_notification_enabled", true)
        _autoRedirectSleepFirstLaunch.value = prefs.getSafeBoolean("auto_redirect_sleep_first_launch", true)
        _backgroundActivityEnabled.value = prefs.getSafeBoolean("enable_background_activity", true)
        ...
        _masterSilentModeEnabled.value = prefs.getSafeBoolean("master_silent_mode", false)
        _backgroundServicesSilentModeEnabled.value = prefs.getSafeBoolean("background_services_silent_mode", false)
        _taskSilentModeEnabled.value = prefs.getSafeBoolean("task_silent_mode", false)
        _habitSilentModeEnabled.value = prefs.getSafeBoolean("habit_silent_mode", false)
        _habitOnScreenReminderEnabled.value = prefs.getSafeBoolean("habit_on_screen_reminder", true)
        _habitNotifReminderEnabled.value = prefs.getSafeBoolean("habit_notif_reminder", true)
        _taskHighNotifEnabled.value = prefs.getSafeBoolean("task_high_notif", true)
        _taskHighDisplayEnabled.value = prefs.getSafeBoolean("task_high_display", true)
        _taskMediumNotifEnabled.value = prefs.getSafeBoolean("task_medium_notif", true)
        _taskMediumDisplayEnabled.value = prefs.getSafeBoolean("task_medium_display", true)
        _taskLowNotifEnabled.value = prefs.getSafeBoolean("task_low_notif", true)
        _taskLowDisplayEnabled.value = prefs.getSafeBoolean("task_low_display", true)
        _taskHighAlarmSoundEnabled.value = prefs.getSafeBoolean("task_high_alarm_sound", false)
        _taskMediumAlarmSoundEnabled.value = prefs.getSafeBoolean("task_medium_alarm_sound", false)
        _taskLowAlarmSoundEnabled.value = prefs.getSafeBoolean("task_low_alarm_sound", false)

        _allDayNotificationEnabled.value = prefs.getSafeBoolean("all_day_notification_enabled", false)
        _allDayNotificationTime.value = prefs.getSafeString("all_day_notification_time", "09:00 AM") ?: "09:00 AM"

        _onThisDayNotificationEnabled.value = prefs.getSafeBoolean("on_this_day_notification_enabled", false)
        _onThisDayNotificationTime.value = prefs.getSafeString("on_this_day_notification_time", "09:00 AM") ?: "09:00 AM"
        _onThisDayOnScreenEnabled.value = prefs.getSafeBoolean("on_this_day_on_screen_enabled", false)
