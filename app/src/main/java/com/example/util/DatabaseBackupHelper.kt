...
<body class="min-h-screen flex flex-col">
    ...
    /**
     * Creates and saves a complete data restoration file directly to the device's public Downloads directory.
     * Returns Pair(success: Boolean, fileName: String).
     */
    suspend fun createAndSaveRestorationBackupToDownloads(context: Context, database: AppDatabase): Pair<Boolean, String> {
        return kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
            try {
                val timeStamp = java.text.SimpleDateFormat("yyyyMMdd_HHmmss", java.util.Locale.getDefault()).format(java.util.Date())
                val prefs = context.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
                val rawUser = prefs.getString("current_username", "")?.takeIf { it.isNotBlank() }
                    ?: prefs.getString("user_email", "")?.substringBefore("@")?.takeIf { it.isNotBlank() }
                    ?: "User"
                val safeUsername = rawUser.replace(Regex("[^a-zA-Z0-9_]"), "_")
                val fileName = "FocusFlow_Restoration_${safeUsername}_$timeStamp.zip"
                ...
                try {
                    ...
                    if (publicDownloads != null) {
                        ...
                        java.io.FileOutputStream(targetFile).use { fos ->
                            val success = exportDataToStream(context, database, fos, isAutoBackup = false)
                            if (success && targetFile.exists() && targetFile.length() > 0) {
                                writtenSuccessfully = true
                            }
                        }

                        // Also place inside Downloads/LifeOS_Backup for auto-discovery
                        try {
                            val lifeOsFolder = java.io.File(publicDownloads, "LifeOS_Backup")
                            if (!lifeOsFolder.exists()) lifeOsFolder.mkdirs()
                            val lifeOsFile = java.io.File(lifeOsFolder, fileName)
                            if (targetFile.exists() && targetFile.length() > 0) {
                                targetFile.copyTo(lifeOsFile, overwrite = true)
                            }
                        } catch (_: Exception) {}

                        // MediaScanner registration
                    ...
                    Log.w(TAG, "Direct public Downloads file export exception: ${e.message}")
                }

                // 2. MediaStore content resolver insertion (ensures full compatibility on Android 10+ scoped storage)
                    ...
                    try {
                        ...
                        if (uri != null) {
                            resolver.openOutputStream(uri)?.use { os ->
                                val ok = exportDataToStream(context, database, os, isAutoBackup = false)
                        ...
                        Log.w(TAG, "MediaStore Downloads insert exception: ${e.message}")
                    }
                }

                Pair(writtenSuccessfully, fileName)
            } catch (e: Exception) {
                Log.e(TAG, "Fatal error creating restoration backup to Downloads", e)
                Pair(false, "")
            }
        }
    }
    ...
    /**
     * Wipes all user data completely from the local device so no trace remains.
     */
    fun wipeAllLocalUserData(context: Context, database: AppDatabase) {
        try {
            ...
            try { com.example.api.BellReceiverService.stopListening() } catch (_: Throwable) {}
            try { com.example.api.FocusLockerManager.stopListening() } catch (_: Throwable) {}
            try { com.example.api.PeerLiveSphereManager.cleanUpListeners() } catch (_: Throwable) {}
            try { com.example.api.DynamicCommandManager.stopListeningToActiveFocusTimer() } catch (_: Throwable) {}
            try { com.example.api.SameUserMultiDeviceSyncManager.stopLiveSync(context) } catch (_: Throwable) {}

            // 2. Dismiss and cancel all notifications
            try {
            ...
            try {
                ...
                Log.e(TAG, "Failed clearing room database tables", e)
            }
            ...
            // 5. Clear all known SharedPreferences
            val knownPrefs = listOf(
                "app_prefs", "app_calendar_prefs", "strict_mode_prefs",
                "countdown_settings_prefs", "app_settings", "pdf_viewer_library_prefs",
                "focus_logs_prefs", "app_lock_prefs", "app_habits_prefs", "app_tasks_prefs",
                "notification_studio_prefs", "widget_prefs", "com.google.android.gms.signin",
                "user_avatar_prefs", "presence_prefs"
            )
            for (pName in knownPrefs) {
                try {
                    context.getSharedPreferences(pName, Context.MODE_PRIVATE).edit().clear().commit()
                } catch (_: Throwable) {}
            }

            // 6. Delete all files in shared_prefs directory
            try {
            ...
            // 7. Delete internal cache, files, codeCache, external cache
            try { context.cacheDir?.deleteRecursively() } catch (_: Throwable) {}
            try { context.codeCacheDir?.deleteRecursively() } catch (_: Throwable) {}
            try { context.filesDir?.deleteRecursively() } catch (_: Throwable) {}
            try {
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
                    context.noBackupFilesDir?.deleteRecursively()
                }
            } catch (_: Throwable) {}
            try { context.externalCacheDir?.deleteRecursively() } catch (_: Throwable) {}
            try {
                context.getExternalFilesDirs(null)?.forEach { dir ->
            ...
            // 8. Sign out of Firebase Auth & Google Account
            try {
            ...
            Log.e(TAG, "Exception in wipeAllLocalUserData", e)
        }
    }
}
