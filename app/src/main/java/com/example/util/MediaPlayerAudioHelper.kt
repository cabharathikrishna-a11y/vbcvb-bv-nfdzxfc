...
class MediaPlayerAudioHelper(private val context: Context) : MediaPlayer.OnPreparedListener, MediaPlayer.OnErrorListener {
    ...
    /**
     * Sets high-speed playback rate (e.g. 1.0x, 1.5x, 2.0x, 2.5x, 3.0x, 3.5x, 4.0x)
     */
    fun setPlaybackSpeed(speed: Float) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            try {
                ...
                Log.d(TAG, "MediaPlayer speed updated to ${speed}x")
            } catch (e: Exception) {
                Log.e(TAG, "Failed to set MediaPlayer playback speed: ${e.message}")
            }
        }
    }
