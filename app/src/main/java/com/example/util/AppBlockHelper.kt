...
object AppBlockHelper {
    ...
    fun initializeCache(context: Context) {
        ...
        val savedBlock = blockPrefs.getSafeStringSet(KEY_BLOCKED_APPS, null) ?: DEFAULT_BLOCKED_APPS
        ...
        val savedStrict = strictPrefs.getSafeStringSet("blocked_packages", emptySet()) ?: emptySet()
        ...
        cachedIgReelsLimitMinutes = igPrefs.getSafeInt("ig_reels_limit_minutes", 0)
        ...
        blockPrefs.registerOnSharedPreferenceChangeListener { _, key ->
            if (key == KEY_BLOCKED_APPS) {
                val updated = blockPrefs.getSafeStringSet(KEY_BLOCKED_APPS, null) ?: DEFAULT_BLOCKED_APPS
                cachedBlockedApps.clear()
        ...
        strictPrefs.registerOnSharedPreferenceChangeListener { _, key ->
            when (key) {
                ...
                "blocked_packages" -> {
                    val updated = strictPrefs.getSafeStringSet("blocked_packages", emptySet()) ?: emptySet()
                    cachedStrictBlockedPackages.clear()
        ...
        igPrefs.registerOnSharedPreferenceChangeListener { _, key ->
            when (key) {
                ...
                "ig_reels_limit_minutes" -> cachedIgReelsLimitMinutes = igPrefs.getSafeInt("ig_reels_limit_minutes", 0)
            }
        }
    ...
    fun getDailyLimitMinutes(context: Context, packageName: String): Int {
        ...
        return getPrefs(context).getSafeInt(PREFIX_DAILY_LIMIT + packageName, defaultLimit)
    }
    ...
    fun getDailyUsageSeconds(context: Context, packageName: String): Int {
        checkAndResetDailyUsageIfNeeded(context)
        return getPrefs(context).getSafeInt(PREFIX_DAILY_USAGE + packageName, 0)
    }
