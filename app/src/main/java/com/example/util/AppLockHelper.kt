...
object AppLockHelper {
    ...
    fun hasBeenUnlockedOnce(context: Context): Boolean {
        return getPrefs(context).getBoolean("has_been_unlocked_once", false)
    }

    fun setUnlockedOnce(context: Context, unlocked: Boolean) {
        getPrefs(context).edit().putBoolean("has_been_unlocked_once", unlocked).apply()
    }

    fun setAppLockEnabled(context: Context, enabled: Boolean) {
        getPrefs(context).edit().putBoolean(KEY_ENABLED, enabled).apply()
        if (!enabled) {
            setUnlockedOnce(context, false)
        }
    }
