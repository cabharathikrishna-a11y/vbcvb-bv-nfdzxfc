...
object TimeEngine {
    ...
    fun initializeNtp(context: Context) {
        ...
        serverTimeOffsetMs = prefs.getSafeLong("firebase_server_time_offset", 0L)
    ...
    /**
     * Fast, thread-safe formatting of epoch timestamp to "yyyy-MM-dd" without SimpleDateFormat allocations.
     */
    fun formatDateToYmd(timeMs: Long = System.currentTimeMillis()): String {
    ...
    /**
     * Highly optimized check to see if two epoch timestamps fall on the same local calendar day.
     * Avoids heavy SimpleDateFormat construction on the main UI/Firebase thread to prevent ANRs.
     */
    fun isUpdatedToday(lastUpdatedMs: Long, nowMs: Long = System.currentTimeMillis()): Boolean {
        if (lastUpdatedMs <= 0L) return false
        val diff = nowMs - lastUpdatedMs
        // Quick boundary filter: if gap is >= 25 hours, impossible to be on same calendar day
        if (diff < -90_000_000L || diff > 90_000_000L) return false

        val tz = java.util.TimeZone.getDefault()
        val nowOffset = tz.getOffset(nowMs)
        val updatedOffset = tz.getOffset(lastUpdatedMs)
        val nowDay = (nowMs + nowOffset) / 86_400_000L
        val updatedDay = (lastUpdatedMs + updatedOffset) / 86_400_000L
        return nowDay == updatedDay
    }
    ...
    fun formatRecordTimeInterval(startTimeStr: String, endTimeStr: String, durationSeconds: Int, totalBreakMs: Long = 0L): String {
        ...
        val startMs = FocusTimerManager.parseTimeToMillisOfDay(startTimeStr)
        val endMs = FocusTimerManager.parseTimeToMillisOfDay(endTimeStr)
        ...
        if (startMs != null && endMs != null) {
            ...
            if (kotlin.math.abs(diffSecs - totalSecs) > 60) {
                ...
                val hour24 = ((correctedStartMs / 3600000L) % 24).toInt()
                val min = (((correctedStartMs % 3600000L) / 60000L)).toInt()
                val sec = (((correctedStartMs % 60000L) / 1000L)).toInt()

                val formattedStart = if (is12Hour) {
                    val amPm = if (hour24 >= 12) "PM" else "AM"
                    val hour12 = if (hour24 == 0) 12 else if (hour24 > 12) hour24 - 12 else hour24
                    String.format(java.util.Locale.US, "%02d:%02d:%02d %s", hour12, min, sec, amPm)
                } else if (startTimeStr.length <= 5) {
                    String.format(java.util.Locale.US, "%02d:%02d", hour24, min)
                } else {
                    String.format(java.util.Locale.US, "%02d:%02d:%02d", hour24, min, sec)
                }
...
object StateReconciliationHelper {
    ...
    fun getTransactionDraft(context: Context): TransactionDraft? {
        val prefs = context.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
        if (!prefs.getSafeBoolean("draft_tx_exists", false)) return null
        return TransactionDraft(
            memberId = prefs.getSafeInt("draft_tx_member_id", -1),
            type = prefs.getSafeString("draft_tx_type", "EXPENSE") ?: "EXPENSE",
            amount = prefs.getSafeFloat("draft_tx_amount", 0.0f).toDouble(),
            note = prefs.getSafeString("draft_tx_note", "") ?: "",
            fromCategory = prefs.getSafeString("draft_tx_from_cat", "") ?: "",
            toCategory = prefs.getSafeString("draft_tx_to_cat", "") ?: "",
            fromAccountId = prefs.getSafeInt("draft_tx_from_acc", -1),
            toAccountId = prefs.getSafeInt("draft_tx_to_acc", -1)
        )
    }
