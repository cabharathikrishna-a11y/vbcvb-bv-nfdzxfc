...
object AppCrashRollbackManager {
    ...
    fun initialize(context: Context) {
        ...
        // Backup current APK asynchronously in background if no working backup exists yet
        Thread {
            try {
    ...
    fun handleCrash(context: Context, thread: Thread, throwable: Throwable) {
        ...
        val failedSet = prefs.getSafeStringSet(KEY_FAILED_VERSION_CODES, emptySet())?.toMutableSet() ?: mutableSetOf()
    ...
    fun checkAndExecuteRollback(context: Context): Boolean {
        ...
        val failedSet = prefs.getSafeStringSet(KEY_FAILED_VERSION_CODES, emptySet()) ?: emptySet()
    ...
    fun markStartupSuccess(context: Context) {
        val currentVersion = AppUpdateManager.getCurrentVersionCode(context)
        val prefs = getPrefs(context)
        val lastWorking = prefs.getSafeInt(KEY_LAST_WORKING_VERSION, -1)
    ...
    fun getFailedVersionCodes(context: Context): Set<Int> {
        val prefs = getPrefs(context)
        val set = prefs.getSafeStringSet(KEY_FAILED_VERSION_CODES, emptySet()) ?: emptySet()
    ...
    fun getLastWorkingVersionCode(context: Context): Int {
        val prefs = getPrefs(context)
        return prefs.getSafeInt(KEY_LAST_WORKING_VERSION, AppUpdateManager.getCurrentVersionCode(context))
    }
