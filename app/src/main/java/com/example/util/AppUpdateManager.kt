...
object AppUpdateManager {
    ...
    fun getPendingUpdateVersion(context: Context): Int {
        val prefs = context.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
        return prefs.getSafeInt("pref_pending_update_version_code", -1)
    }
    ...
    fun getReadyApkPath(context: Context): String? {
        val prefs = context.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
        return prefs.getSafeString("pref_ready_apk_path", null)
    }
    ...
    fun getRunningFirebaseVersion(context: Context): Int {
        val prefs = context.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
        val saved = prefs.getSafeInt("pref_running_firebase_version", -1)
        if (saved == -1) {
    ...
    fun checkAndNotifyUpgradeComplete(context: Context): Int? {
        return try {
            val prefs = context.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
            val lastKnown = prefs.getSafeInt("pref_last_known_version_code", -1)
            val lastCelebrated = prefs.getSafeInt("pref_last_celebrated_version", -1)
            ...
            val lastKnownUpdateTime = prefs.getSafeLong("pref_last_known_update_time", 0L)
            ...
            val installTarget = prefs.getSafeInt("pref_install_target_version", -1)
            ...
            Log.e(TAG, "Error in checkAndNotifyUpgradeComplete", e)
            null
        }
    }
