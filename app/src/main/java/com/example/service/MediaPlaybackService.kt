    ...
    override fun onCreate() {
        ...
        localExoPlayer.addListener(object : Player.Listener {
            override fun onIsPlayingChanged(isPlaying: Boolean) {
                val currentTrackTitle = localExoPlayer.currentMediaItem?.mediaMetadata?.title?.toString() ?: "In-Built Audio Track"
                val currentArtist = localExoPlayer.currentMediaItem?.mediaMetadata?.artist?.toString() ?: "Life OS Audio"
                val currentSpeed = localExoPlayer.playbackParameters.speed
                com.example.util.UnifiedMediaNotificationManager.updateState(
                    context = applicationContext,
                    title = currentTrackTitle,
                    subtitle = "$currentArtist • In-Built Player",
                    playing = isPlaying,
                    speed = currentSpeed,
                    playerType = com.example.util.ActivePlayerType.IN_BUILT
                )
            }

            override fun onMediaItemTransition(mediaItem: MediaItem?, reason: Int) {
                val currentTrackTitle = mediaItem?.mediaMetadata?.title?.toString() ?: "In-Built Audio Track"
                val currentArtist = mediaItem?.mediaMetadata?.artist?.toString() ?: "Life OS Audio"
                val currentSpeed = localExoPlayer.playbackParameters.speed
                com.example.util.UnifiedMediaNotificationManager.updateState(
                    context = applicationContext,
                    title = currentTrackTitle,
                    subtitle = "$currentArtist • In-Built Player",
                    playing = localExoPlayer.isPlaying,
                    speed = currentSpeed,
                    playerType = com.example.util.ActivePlayerType.IN_BUILT
                )
            }
        ...
        // Listen for notification commands if active player is IN_BUILT
        kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.Main).launch {
            com.example.util.UnifiedMediaNotificationManager.actionCommands.collect { cmd ->
                if (com.example.util.UnifiedMediaNotificationManager.activePlayerType == com.example.util.ActivePlayerType.IN_BUILT) {
                    when (cmd) {
                        is com.example.util.MediaActionCommand.PlayPause -> {
                            if (localExoPlayer.isPlaying) localExoPlayer.pause() else localExoPlayer.play()
                        }
                        is com.example.util.MediaActionCommand.Rewind -> {
                            localExoPlayer.seekTo(Math.max(0L, localExoPlayer.currentPosition - 10000L))
                        }
                        is com.example.util.MediaActionCommand.FastForward -> {
                            localExoPlayer.seekTo(Math.min(localExoPlayer.duration, localExoPlayer.currentPosition + 10000L))
                        }
                        is com.example.util.MediaActionCommand.ChangeSpeed -> {
                            localExoPlayer.setPlaybackSpeed(cmd.speed)
                        }
                        is com.example.util.MediaActionCommand.NextOrSkipAd -> {
                            if (localExoPlayer.hasNextMediaItem()) {
                                localExoPlayer.seekToNextMediaItem()
                            }
                        }
