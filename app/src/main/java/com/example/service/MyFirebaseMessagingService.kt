...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        if (data.isNotEmpty()) {
            ...
            if (messageType == "PEER_FOCUS_START" || messageType == "FRIEND_FOCUS_START" || messageType == "peer_focus" || messageType == "friend_started_focusing") {
                ...
                if (senderId.isNotBlank() && !com.example.api.PeerLiveSphereManager.isSelfUser(applicationContext, senderId)) {
                    ...
                    Log.d(TAG, "Suppressed peer focus start notification: sender is self ($senderId)")
                }
                return
            }
        ...
        val senderId = data["sender_id"] ?: data["friend_id"] ?: data["email"] ?: ""
        if (com.example.api.PeerLiveSphereManager.isSelfUser(applicationContext, senderId)) {
            if (finalTitle.contains("focusing", ignoreCase = true) || finalBody.contains("focusing", ignoreCase = true)) {
                Log.d(TAG, "Suppressed self focus notification in fallback handler")
                return
            }
        }
    ...
    private fun sendNotification(title: String, messageBody: String) {
        if (!com.example.util.NotificationSettingsManager.isGeneralFcmEnabled(this)) {
            Log.d(TAG, "Suppressed FCM notification: disabled in notification settings")
            return
        }
