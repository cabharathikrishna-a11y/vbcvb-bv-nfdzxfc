...
class KeepAliveService : Service() {
    ...
    override fun onCreate() {
        ...
        serviceScope.launch {
            var lastUpdateMin = -1
            while (isActive) {
                if (!com.example.widget.WidgetManager.hasTotalFocusWidgets(applicationContext)) {
                    delay(30000L)
                    continue
                }
        ...
        serviceScope.launch {
            com.example.api.PeerLiveSphereManager.peerLiveStates
                .sample(8000L)
                .collect {
                    if (com.example.widget.WidgetManager.hasFriendsFocusWidgets(applicationContext)) {
        ...
        serviceScope.launch {
            while (isActive) {
                if (!com.example.widget.WidgetManager.hasPhotoShowerWidgets(applicationContext)) {
                    delay(60000L)
                    continue
                }
                ...
                val intervalSec = prefs.getSafeInt("journal_photo_widget_interval_sec", 30).coerceAtLeast(30)
                ...
                try {
                    ...
                    if (!focusGuardDisabled && !intercepted) {
                        ...
                        if (monitoredApps.contains(foregroundPackage)) {
                            val blockedApps = strictPrefs.getSafeStringSet("blocked_packages", emptySet()) ?: emptySet()
    ...
    private fun checkInactivityAndNotify() {
        try {
            ...
            if (isLocalFocusing) {
                ...
                var lastTime = prefs.getSafeLong("last_focus_activity_timestamp_ms", 0L)
                if (lastTime == 0L) {
    ...
    private fun sendInactivityEncouragementNotification() {
        if (!com.example.util.NotificationSettingsManager.isStudyMotivationalEnabled(this)) {
            Log.d("KeepAliveService", "Suppressed inactivity encouragement notification: disabled in notification settings")
            return
        }
                ...
                try {
                    ...
                    if (enabled) {
                        ...
                        if (isWithinWakeupWindow) {
                            val lastMs = prefs.getSafeLong("last_water_reminder_time_ms", 0L)
    ...
    private fun triggerWaterReminderNotification() {
        if (!com.example.util.NotificationSettingsManager.isWaterReminderEnabled(this)) {
            Log.d("KeepAliveService", "Suppressed water reminder notification: disabled in notification settings")
            return
        }
