...
object DevicePresenceManager {
    ...
    /**
     * Checks whether a user identifier (key, peerEmail, userId) belongs to the current user (Me).
     */
    fun isMeUser(
        key: String,
        ...
        if (cleanKey == "admin") return true

        val cleanPeerEmail = (peerEmail ?: "").lowercase().trim()
        val cleanUserId = (userId ?: "").lowercase().trim()
        val cleanMyEmail = myEmail.lowercase().trim()
        val cleanMyUsername = myUsername.lowercase().trim()
        val activeEmail = DynamicCommandManager.activeEmail.lowercase().trim()

        val sanitizedKey = sanitizeEmail(cleanKey)
        val sanitizedPeerEmail = sanitizeEmail(cleanPeerEmail)
        val sanitizedUserId = sanitizeEmail(cleanUserId)
        val sanitizedMyEmail = sanitizeEmail(cleanMyEmail)
        val sanitizedMyUsername = sanitizeEmail(cleanMyUsername)
        val sanitizedActiveEmail = sanitizeEmail(activeEmail)

        val myPrefix = if (cleanMyEmail.contains("@")) cleanMyEmail.substringBefore("@") else cleanMyUsername
        val activePrefix = if (activeEmail.contains("@")) activeEmail.substringBefore("@") else ""

        // Check key
        if (cleanKey.isNotEmpty()) {
            if (cleanKey == cleanMyEmail || cleanKey == cleanMyUsername || cleanKey == activeEmail) return true
            if (sanitizedKey.isNotEmpty() && (sanitizedKey == sanitizedMyEmail || sanitizedKey == sanitizedMyUsername || sanitizedKey == sanitizedActiveEmail)) return true
            if (myPrefix.length >= 3 && (cleanKey == myPrefix || sanitizedKey == myPrefix)) return true
            if (activePrefix.length >= 3 && (cleanKey == activePrefix || sanitizedKey == activePrefix)) return true
        }

        // Check peerEmail
        if (cleanPeerEmail.isNotEmpty()) {
            if (cleanPeerEmail == cleanMyEmail || cleanPeerEmail == cleanMyUsername || cleanPeerEmail == activeEmail) return true
            if (sanitizedPeerEmail.isNotEmpty() && (sanitizedPeerEmail == sanitizedMyEmail || sanitizedPeerEmail == sanitizedMyUsername || sanitizedPeerEmail == sanitizedActiveEmail)) return true
        }

        // Check userId
        if (cleanUserId.isNotEmpty()) {
            if (cleanUserId == cleanMyEmail || cleanUserId == cleanMyUsername || cleanUserId == activeEmail) return true
            if (sanitizedUserId.isNotEmpty() && (sanitizedUserId == sanitizedMyEmail || sanitizedUserId == sanitizedMyUsername || sanitizedUserId == sanitizedActiveEmail)) return true
        }
    ...
    fun registerPresence(context: Context, email: String) {
        ...
        try {
            ...
            val updatePresenceWithToken = { fcmToken: String ->
                    ...
                    if (task.isSuccessful) {
                        ...
                        kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.IO).launch {
                            try {
                                ...
                                Log.e(TAG, "Error updating post-presence stats for $email", e)
                            }
                        }
                    } else {
                        Log.e(TAG, "Failed to update device presence for $deviceKey", task.exception)
                    }
                }
            }
